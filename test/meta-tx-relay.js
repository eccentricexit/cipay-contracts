...
describe('Test MetaTxRelay', () => {
  ...
  it('should succeed when the sender has enough balance and the sender has a large enough allowance', async () => {
    ...
    const chainId = await metaTxRelay.provider.getNetwork().then(n => n.chainId);
    ...
    expect(balanceBefore.lt(balanceAfter), `Receiver balance should've increased`).to.be.true
  })
})
